...
return this;},showSlide:function(slide){var $slideshow=this,data=$slideshow.data('rsf_slideshow'),containerWidth=$slideshow.width(),containerHeight=$slideshow.height();RssPrivateMethods._trigger($slideshow,'rsPreTransition');$slideshow.children('img:first').css('z-index',0);var whenLoaded=function(img){var $img=$(img);$img.addClass('rsf-slideshow-image');var whenDimensions=function(width,height){RssPrivateMethods._trigger($slideshow,'rsImageReady');var leftOffset=Math.ceil((containerWidth/2)-(width/2));var topOffset=Math.ceil((containerHeight/2)-(height/2));$img.css({left:leftOffset});$img.css({top:topOffset});if(slide.image_title){$img.attr('title',slide.image_title);}
...
data.slides.push(slide);}},_removeSlide:function($slideshow,key){$slideshow.data('rsf_slideshow').slides.splice(key,1);},_transitionWith:function($slideshow,$slide,effect){var data=$slideshow.data('rsf_slideshow'),effect_iteration='random';if(typeof effect==='object'&&effect.iteration&&effect.effects){effect_iteration=effect.iteration;effect=effect.effects;}
