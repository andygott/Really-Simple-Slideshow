...
else if(slide_keys instanceof Array){slide_keys.sort(function(a,b){return b-a;});var removed=[];return this.each(function(){for(var i=0,len=slide_keys.length;i<len;i++){if($.inArray(slide_keys[i],removed)===-1){private._removeSlide($(this),slide_keys[i]);removed.push(slide_keys[i]);}}});}
else{return this.each(function(){private._removeSlide($(this),slide_keys);});}},getSlideData:function(key){if(key===undefined){return this.data('rsf_slideshow').slides;}
